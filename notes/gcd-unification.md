...
Of course, how does the Stern-Brocot fit into this picture?
...
Interestingly, by the Euler-Lagrange theorem, the elements in `SL(2,N)`
...
Hmm, actually we don't get all quadratic irrationals from this procedure:
quadratic irrationals have eventually periodic continued fractions, whereas
these quadratic irrationals must be periodic from the very beginning. Also there
is an "interesting" matrix operator lurking here: the elements that commute with
a given `m ∈ SL(2,N)` is generated by the forward orbit of `m' ∈ SL(2,N)`, where
`m' ^ i = m` for the largest possible `i` that has a solution.  Surely `m'`
corresponds to something that has already been studied.
...
monoid elements can collapse into the same element. Assuming that my intuition
is correct that "if `M` is non-trivial and a submonoid of `G`, then `M`
contains non-trivial pairs of elements that commute", that might not imply
that `G` contains non-trivial pairs of elements that commute.
